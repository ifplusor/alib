 ...
 */
#ifndef __ALIB_OBJECT_H__
#define __ALIB_OBJECT_H__

// use tagged pointer
#include "../memory/taggedpointer.h"
...
#define aobj_alloc_with_ex(type, init, data, size) _aobj_alloc(sizeof(type) + (size), init, data)

#define aobj_init(type, ptr) _aobj_init(ptr, &_aobj_##type##_meta)

// reference count operator
#define _retain(id) _aobj_retain(id)
#define _release(id) _aobj_release(id)

#define _instanceof(type, id) _aobj_instanceof(id, _aobj_##type##_identifier)
...
#define aclass(type, ...) \
struct _aobj_##type##_; \
typedef struct _aobj_##type##_ type##_s; \
typedef type##_s *type##_t; \
struct _aobj_##type##_ { \
  uint32_t magic; \
  __VA_ARGS__ \
}; \
...
#define afunc_delc(type, message, rtype, ...) ALIB_EXP_DECL rtype type##_##message(__VA_ARGS__)

#define afunc_defn(type, message, rtype, ...) ALIB_EXP_DEFN rtype type##_##message(__VA_ARGS__)

#define _alloc(type, factory, ...) type##_##factory(__VA_ARGS__)

#define _(type, id, message, ...) type##_##message(id, ##__VA_ARGS__)

#endif  // __ALIB_OBJECT_H__
